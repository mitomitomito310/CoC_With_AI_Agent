...
1. `python tools/coc_import.py <source> --scenario-id <id> --title <title> --location <location> --participant <id>`で原本を `scenarios/<scenario_id>/source/` に読み取り専用コピーし、SHA-256を記録する。既存workspaceへの上書きを許可しない。
...
7. handout原本は`source/handouts`に不変保持し、公開コピーの`unreleased/released`状態とは別に記録する。sheet、例、シナリオ本文を普遍ルールとして自動抽出しない。
