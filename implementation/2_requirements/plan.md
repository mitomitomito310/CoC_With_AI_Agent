...
## 2026-07-25 Quick-Start根拠資料の追補

- フェーズ状態は**Phase 2承認済み・Phase 3へ引き渡し済み**のままとする。この根拠資料の精緻化は承認を取り消さず、フェーズゲートも進めない。
- `NewCoC-QS_200228.pdf`全43ページを`SRC-QS-JA-20200228`として採用し、ページ単位のルールIDとQuick-Start／コアルールの境界を`rule_ledger.md`へ記録した。
- 現時点でPhase 3から戻す新たな判断事項はない。対象外または完全同値の動作を選ぶ必要が生じた場合は、実装前に未決事項としてここへ戻す。ユーザー承認なしにフェーズ状態を変更しない。
