...
- **FR-RUL-01:** ユーザー探索者は有効な参照プロファイルで裏づけられた手順で作成し、質問分割、候補提示、自動計算で操作を支援する。`quick_start_7e`では固定能力値配分、能力値・技能の2分の1/5分の1、派生値、職業技能、個人的興味、バックストーリー、装備を段階的かつ再開可能に処理する。「フル手順」は完全版の全オプションを意味せず、完全版を名乗るには別の正規参照元を必要とする。
- **FR-RUL-02:** ダイス結果、適用した技能・能力・現在値、難易度、全ての生ダイス、採用値、ボーナス/ペナルティと理由、成功度、対抗者と同値処理、プッシュ可否・失敗時結果、参照根拠、状態差分を追跡できる。
...
- **FR-RUL-05:** 参照元は`rules_profile`、版、文書識別子、ページまたは節、対応メカニクス、完全性を台帳化する。`quick_start_7e`、`full_7e`、`house_rule`を混同せず、有効な参照元にない処理は`unsupported_by_active_source`として停止または暫定裁定へ送る。
- **FR-RUL-06:** SAN、戦闘、負傷・治療、MP、成長などの継続状態は、ロール結果だけでなく現在値、最大値、状態フラグ、発生時刻、回復期限、根拠、変更履歴としてcheckpointへ保存する。
- **FR-RUL-07:** 戦闘はDEX順、近接の回避/応戦、火器準備・射程・複数射撃・遮蔽、イクストリーム・ダメージ、数的不利、戦闘マヌーバー、重傷・瀕死を区別する。役割や武器種で同値・ダメージ規則が変わるため、単一の成功/失敗フラグへ縮約しない。
- **FR-RUL-08:** 一時的狂気やKeeperの秘匿ロールは、ユーザーに見せない機械結果と観測可能な描写を分離する。プレイヤー探索者の行動制御を伴う場合は、規則上の期間・根拠を記録し、必要以上に代理決定しない。
